  ...
  <div class="tour-detail-page">
    ...
    <div class="container">
      ...
      <div class="breadcrumb">
        <NuxtLink to="/tours">{{ t('nav.tours') }}</NuxtLink>
        <Icon name="heroicons:chevron-right" />
        <span>{{ tour?.title || 'Carregando...' }}</span>
      </div>
      
      <!-- Loading State -->
      <div v-if="loading" class="tour-detail-page__loading">
        <div class="tour-detail-page__skeleton">
          <div class="tour-detail-page__skeleton-hero"></div>
          <div class="tour-detail-page__skeleton-content">
            <div class="tour-detail-page__skeleton-title"></div>
            <div class="tour-detail-page__skeleton-subtitle"></div>
            <div class="tour-detail-page__skeleton-meta"></div>
          </div>
        </div>
      </div>
      
      <!-- Error State -->
      <div v-else-if="error" class="tour-detail-page__error">
        <Icon name="heroicons:exclamation-triangle" />
        <h2>{{ t('common.error') }}</h2>
        <p>{{ error }}</p>
        <NuxtLink to="/tours" class="tour-detail-page__back">
          {{ t('common.backToTours') }}
        </NuxtLink>
      </div>
      
      <!-- Tour Content -->
      <div v-else-if="tour" class="tour-detail-page__content">
        <!-- Hero Section -->
    <div class="tour-detail-page__hero">
      <div class="tour-detail-page__hero-image">
            <!-- Galeria de Imagens -->
            <div class="tour-detail-page__image-gallery">
              <!-- Imagem Principal -->
              <div 
                class="tour-detail-page__main-image" 
              ...
              >
                <img 
                  ...
                  @click="openLightbox"
                  class="tour-detail-page__main-image-img"
                  style="object-fit: contain !important; object-position: center !important;"
                  loading="lazy"
                />
                <button v-if="images.length > 1" class="tour-detail-page__nav prev" @click="prevImage" aria-label="Imagem anterior">‹</button>
                <button v-if="images.length > 1" class="tour-detail-page__nav next" @click="nextImage" aria-label="Próxima imagem">›</button>
                <div v-if="images.length > 1" class="tour-detail-page__image-counter">{{ currentImageIndex + 1 }} / {{ images.length }}</div>
              </div>
              
              <!-- Miniaturas -->
              <div v-if="images.length > 1" class="tour-detail-page__thumbnails" role="listbox">
                <button
                  v-for="(img, i) in images"
                  :key="i"
                  class="tour-detail-page__thumbnail"
            ...
            <!-- Badges -->
            <div v-if="tour.badge" class="tour-detail-page__badge">{{ tour.badge }}</div>
            <div v-if="tour.hasPromotion && tour.discount_percentage" class="tour-detail-page__discount-badge">
              -{{ tour.discount_percentage }}% OFF
            </div>
          </div>
          
          <div class="tour-detail-page__hero-content">
            <h1 class="tour-detail-page__title">{{ tour.title }}</h1>
            
            <div class="tour-detail-page__meta">
              <div v-if="tour.location" class="tour-detail-page__location">
                <Icon name="heroicons:map-pin" />
                <span>{{ tour.location }}</span>
              </div>
              
              <div v-if="tour.rating" class="tour-detail-page__rating">
                <Icon name="heroicons:star-solid" />
                <span>{{ tour.rating }}</span>
                <span v-if="tour.reviewsCount">({{ tour.reviewsCount }})</span>
              </div>
              
              <!-- Schedule Info -->
              <div v-if="tour.check_in_time || tour.check_out_time" class="tour-detail-page__schedule">
                <div v-if="tour.check_in_time" class="tour-detail-page__schedule-item">
                  <Icon name="heroicons:arrow-right-start-on-rectangle" />
                  <span>{{ t('tour.checkIn') }}: {{ tour.check_in_time }}</span>
                </div>
                <div v-if="tour.check_out_time" class="tour-detail-page__schedule-item">
                  <Icon name="heroicons:arrow-right-end-on-rectangle" />
                  <span>{{ t('tour.checkOut') }}: {{ tour.check_out_time }}</span>
                </div>
              </div>
            </div>
            ...
            <button v-if="price > 0" class="tour-detail-page__cta" @click="scrollToBooking">Reservar agora</button>
          </div>
        </div>
        
        <!-- Main Content -->
        <div class="tour-detail-page__main">
          <div class="tour-detail-page__info">
            <!-- Description -->
            <div class="tour-detail-page__section">
              <h2>{{ t('tour.description') }}</h2>
              <!-- If HTML, render sanitized HTML -->
              <div v-if="descIsHtml" v-html="safeHtml" class="tour-detail-page__description-html"></div>
              <!-- Otherwise, render plain text with line breaks -->
              <p v-else class="tour-detail-page__description-text">{{ rawDesc }}</p>
            </div>
            
            <!-- Details -->
            <div class="tour-detail-page__section">
              <h2>{{ t('tour.details') }}</h2>
              <div class="tour-detail-page__details-grid">
                <div v-if="tourDetails?.duration" class="tour-detail-page__detail-item">
                  <Icon name="heroicons:clock" />
                  <div>
                    <h3>{{ t('tour.duration') }}</h3>
                    <p>{{ tourDetails.duration }}</p>
                  </div>
                </div>
                
                <div v-if="formattedCapacity && formattedCapacity !== '—'" class="tour-detail-page__detail-item">
                  <Icon name="heroicons:users" />
                  <div>
                    <h3>{{ t('tour.capacity') }}</h3>
                    <p>{{ formattedCapacity }}</p>
                  </div>
                </div>
                
                <div v-if="tourDetails?.minNights" class="tour-detail-page__detail-item">
                  <Icon name="heroicons:calendar" />
                  <div>
                    <h3>{{ t('tour.minNights') }}</h3>
                    <p>{{ tourDetails?.minNights }} {{ t('common.nights') }}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="tour-detail-page__section">
              <h2>{{ t('tour.importantInfo') }}</h2>
              <div class="tour-detail-page__info-grid">
                <div v-if="tour.meeting_point" class="tour-detail-page__info-item">
                  <Icon name="heroicons:map-pin" />
                  <div>
                    <h3>{{ t('tour.meetingPoint') }}</h3>
                    <p>{{ tour.meeting_point }}</p>
                  </div>
                </div>
                
                <div v-if="tour.cancellation_policy" class="tour-detail-page__info-item">
                  <Icon name="heroicons:shield-check" />
                  <div>
                    <h3>{{ t('tour.cancellationPolicy') }}</h3>
                    <p>{{ tour.cancellation_policy }}</p>
                  </div>
                </div>
                
                <div v-if="tour.difficulty_level" class="tour-detail-page__info-item">
                  <Icon name="heroicons:chart-bar" />
                  <div>
                    <h3>{{ t('tour.difficultyLevel') }}</h3>
            ...
            <!-- Includes -->
            <div v-if="tour.includes && tour.includes.length > 0" class="tour-detail-page__section">
              <h2>{{ t('tour.includes') }}</h2>
              <ul class="tour-detail-page__includes">
            ...
            <!-- Excludes -->
            <div v-if="tour.excludes && tour.excludes.length > 0" class="tour-detail-page__section">
              <h2>{{ t('tour.excludes') }}</h2>
              <ul class="tour-detail-page__excludes">
                <li v-for="item in tour.excludes" :key="item">
            ...
            <!-- Itinerary -->
            <div v-if="tour.itinerary && tour.itinerary.length > 0" class="tour-detail-page__section">
              <h2>{{ t('tour.itinerary') }}</h2>
              <ol class="tour-detail-page__itinerary">
                <li v-for="(step, index) in tour.itinerary" :key="index">
                  <span class="tour-detail-page__step-number">{{ index + 1 }}</span>
                  {{ step }}
                </li>
              </ol>
            </div>
        ...
        <!-- Booking Sidebar -->
          <div class="tour-detail-page__booking">
            <div class="tour-detail-page__booking-card" id="booking">
              <h3>{{ t('tour.bookNow') }}</h3>
              ...
              <!-- Calendar -->
              <div class="tour-detail-page__calendar">
                <CalendarAvailability
                  mode="tour"
                  :min-nights="tourDetails?.minNights"
                  :max-nights="tourDetails?.maxNights"
                  :allow-range="tourDetails?.allowRange"
                  ...
                  :base-price="(tour as any)?.price || (tour as any)?.current_price || 0"
              ...
              <div v-if="selectedDate || selectedRange" class="tour-detail-page__selected-dates">
                <h4>Datas Selecionadas</h4>
                <div class="tour-detail-page__date-info">
                  <div v-if="selectedDate" class="tour-detail-page__date-item">
                    <Icon name="heroicons:calendar-days" />
                    <span>{{ formatDate(selectedDate) }}</span>
                  </div>
                  <div v-if="selectedRange" class="tour-detail-page__date-item">
                    <Icon name="heroicons:calendar-days" />
                    <span>{{ formatDate(selectedRange.from) }} - {{ formatDate(selectedRange.to) }}</span>
                    <span class="tour-detail-page__nights">({{ selectedRange.nights }} {{ t('common.nights') }})</span>
                  </div>
                </div>
              </div>
              
              <!-- Participants -->
              <div class="tour-detail-page__participants">
                <h4>{{ t('tour.participants') }}</h4>
                <div class="tour-detail-page__participant-controls">
                  <div class="tour-detail-page__participant-group">
                    <label>{{ t('common.adults') }}</label>
                    <div class="tour-detail-page__counter">
                  ...
                  <div v-if="tour" class="tour-detail-page__participant-progress">
                    <div class="tour-detail-page__progress-bar">
                      <div 
                        class="tour-detail-page__progress-fill"
                        :style="{ width: `${(totalParticipants / productRules.maxParticipants) * 100}%` }"
                      ></div>
                    </div>
                    <div class="tour-detail-page__progress-text">
      ...
      <div 
        v-if="participantValidation.message && tour" 
        class="tour-detail-page__participant-feedback"
        :class="`tour-detail-page__participant-feedback--${participantValidation.type}`"
      >
        <Icon 
        ...
        <span>{{ participantValidation.message }}</span>
                  ...
                  <div v-else-if="productRules.maxParticipants < 10" class="tour-detail-page__participant-info">
                    <Icon name="heroicons:information-circle" />
                    <span>Máximo {{ productRules.maxParticipants }} participantes</span>
                  </div>
                  
                  <!-- Link para grupos grandes -->
                  <div v-if="totalParticipants >= productRules.maxParticipants * 0.8" class="tour-detail-page__large-group">
                    <div class="tour-detail-page__large-group-content">
                      <Icon name="heroicons:user-group" class="tour-detail-page__large-group-icon" />
                      <div class="tour-detail-page__large-group-text">
                        <h5>Grupo grande?</h5>
                    ...
                    <div class="tour-detail-page__whatsapp-buttons">
                      <a 
                        href="https://wa.me/5522997892414?text=Olá! Gostaria de informações sobre reservas para grupos grandes para o tour: {{ tour?.title }}"
                        class="tour-detail-page__whatsapp-button"
                        target="_blank"
                        rel="noopener"
                      ...
                      <a 
                        href="https://wa.me/5522998677244?text=Olá! Gostaria de informações sobre reservas para grupos grandes para o tour: {{ tour?.title }}"
                        class="tour-detail-page__whatsapp-button"
                        target="_blank"
                        rel="noopener"
              ...
              <!-- Auto-save Status -->
              <div class="tour-detail-page__auto-save-status">
                <div class="tour-detail-page__save-indicator" :class="`tour-detail-page__save-indicator--${saveStatus.color}`">
                ...
                <button 
                  ...
                  @click="clearProgress"
                  class="tour-detail-page__clear-save"
              ...
              <!-- Price Summary -->
              <div class="tour-detail-page__price-summary">
                <PriceSummary
                  key="price-summary"
                  mode="tour"
              ...
              <!-- Price Comparison -->
              <div class="tour-detail-page__price-comparison">
                <PriceComparison
    ...
    <!-- Lightbox -->
    <Teleport to="body">
      <div v-if="showLightbox" class="tour-detail-page__lightbox" @click="closeLightbox">
        <div class="tour-detail-page__lightbox-content" @click.stop>
          <button @click="closeLightbox" class="tour-detail-page__lightbox-close">
            <Icon name="heroicons:x-mark" />
          </button>
          
          <button @click="prevImage" class="tour-detail-page__lightbox-nav tour-detail-page__lightbox-prev">
            <Icon name="heroicons:chevron-left" />
          </button>
          
          <div class="tour-detail-page__lightbox-image">
            <img :src="selectedImage" :alt="tour?.title || ''" />
          </div>
          
          <button @click="nextImage" class="tour-detail-page__lightbox-nav tour-detail-page__lightbox-next">
            <Icon name="heroicons:chevron-right" />
          </button>
          
          <div class="tour-detail-page__lightbox-counter">
            {{ currentImageIndex + 1 }} / {{ images.length }}
          </div>
          
          <div class="tour-detail-page__lightbox-thumbnails">
            <button
              v-for="(image, index) in images"
              :key="index"
              @click="selectImage(index)"
              :class="['tour-detail-page__lightbox-thumbnail', { active: index === currentImageIndex }]"
            >
              <img :src="image" :alt="`${tour?.title || ''} - Imagem ${index + 1}`" />
            </button>
          </div>
        </div>
      </div>
    ...
    <!-- Trust Signals -->
    <div v-if="tour" class="tour-detail-page__trust">
      <TrustSignals />
    </div>

    <!-- Reviews Section -->
    <div v-if="tour" class="tour-detail-page__reviews">
      <ClientOnly>
        <ReviewsDisplay :product-id="tour.id" product-type="tour" />
      </ClientOnly>
    </div>
  </div>
...
const { getFeaturedTours, normalizeTourData } = useFeatured()
...
const tour = ref<FeaturedItem | null>(null)
...
const { 
  ...
  acceptOffer, 
  declineOffer 
} = useExitIntent(tour.value?.id || '')

// Lógica de renderização condicional para descrição
const rawDesc = computed(() => tour.value?.description ?? '')
...
const images = computed<string[]>(() => {
  if (tour.value?.images && tour.value.images.length > 0) {
    ...
    if (typeof tour.value.images[0] === 'object' && tour.value.images[0].url) {
      return tour.value.images.map((img: any) => img.url)
    }
    // Se images é um array de strings, usar diretamente
    return tour.value.images
  }
  return tour.value?.image ? [tour.value.image] : []
})
...
const price = computed<number>(() => {
  ...
  const tourData = tour.value as any
...
const openBookingConfirmation = () => {
  if (!selectedDate.value || !tour.value || !pricingBreakdown.value) {
...
const loadPriceComparison = async () => {
  if (!tour.value || !pricingBreakdown.value) return
...
const getMainImageAltText = (): string => {
  if (!tour.value) return 'Imagem do tour'
  
  const tourTitle = tour.value.title || 'Tour'
  const location = tour.value.location ? ` em ${tour.value.location}` : ''
...
const getThumbnailAltText = (index: number): string => {
  if (!tour.value) return `Miniatura ${index + 1}`
...
const loadTour = async () => {
  try {
    ...
    // Buscar tour específico via API /products/extended/{uuid}
    // Primeiro tentar buscar na lista para encontrar o UUID
    const toursResult = await getFeaturedTours({ limit: 100 })
    const foundTour = toursResult.data.value.find((t: any) => t.slug === slug || t.id === slug)
    
    if (!foundTour) {
      error.value = 'Tour não encontrado'
      return
    }
    
    // Agora buscar detalhes completos via /products/extended/{uuid}
    const tourUuid = foundTour.uuid || foundTour.id
    
    // Usar getTourDetails que chama /products/extended/{uuid}
    const { getTourDetails } = useAvailability()
    const tourDetailsData = await getTourDetails(tourUuid)
    
    // Normalizar com useProductDetails para garantir campos corretos
    const { normalizeTourDetails } = useProductDetails()
    const normalizedTour = normalizeTourDetails(tourDetailsData)
    
    tour.value = normalizedTour
    tourDetails.value = normalizedTour
    
    // Carregar detalhes do tour (disponibilidade, etc.)
    // Usar UUID real do tour encontrado (já declarado acima)
    if (tourUuid && tour.value) {
      // Detectar se é um produto que permite range selection (acomodações)
      const isAccommodation = tour.value.type === 'stay' || 
                             tour.value.category?.toLowerCase().includes('accommodation') ||
                             tour.value.category?.toLowerCase().includes('hotel') ||
                             tour.value.category?.toLowerCase().includes('pousada')
      
      tourDetails.value = {
        id: slug,
        title: tour.value.title,
        description: tour.value.description || 'Descrição não disponível',
        duration: tour.value.duration || '',
        capacity: tour.value.capacity ? {
          min: tour.value.capacity,
          max: tour.value.capacity
        } : {
          min: 1,
          max: 10
        },
        minNights: isAccommodation ? 2 : (tour.value.minNights || 1),
        maxNights: isAccommodation ? 30 : (tour.value.maxNights || 1),
        allowRange: isAccommodation,
        includes: tour.value.includes || [],
        excludes: tour.value.excludes || [],
        itinerary: tour.value.itinerary || [],
        policies: []
      }
    } else {
      // Fallback se tour.value for null
      tourDetails.value = {
        id: slug,
    ...
    error.value = err.message || 'Erro ao carregar tour'
...
const loadAvailability = async () => {
  try {
    ...
    const tourId = tour.value?.uuid || tour.value?.id
    if (!tourId) {
...
const loadPricing = async () => {
  ...
  try {
    ...
    const tourUuid = tour.value?.uuid || tour.value?.id || slug
...
const updatePricingBreakdownWithChildrenAges = async (childrenAges: any[]) => {
  if (!basePricing.value || !tour.value) return
  ...
  if (childrenAges && childrenAges.length > 0) {
    try {
      ...
      const basePrice = (tour.value as any)?.price || (tour.value as any)?.current_price || 0
      
      // Converter array de objetos para array de strings
      const ageStrings = childrenAges.map(child => child.age)
      childrenTotal = calculateTotalChildrenPrice(basePrice, ageStrings, tour.value.id)
    } catch (error) {
...
watch(() => pricingBreakdown.value, () => {
  if (pricingBreakdown.value && tour.value) {
    loadPriceComparison()
  }
}, { deep: true })
...
const handleUrgencyDismiss = () => {
  console.log('🚨 Urgency banner dismissed')
}

const handleUrgentMessage = (message: string) => {
  console.log('🚨 Urgent message:', message)
...
const applySavedDates = () => {
  ...
  const currentProgress = restoreProgress(tour.value?.id)
...
onMounted(() => {
  ...
  document.querySelectorAll('.tour-detail-page__cta, .price-summary__button')
...
// Computed para SEO
const seoTitle = computed(() => tour.value?.title ?? 'Passeio')
const seoDescription = computed(() => {
  const desc = tour.value?.subtitle || tour.value?.description || ''
  return desc.length > 160 ? desc.slice(0, 160) + '...' : desc
})
const seoImage = computed(() => {
  const rawImg = tour.value?.image ?? '/og/default.jpg'
  return rawImg.startsWith('http') ? rawImg : `${siteUrl}${rawImg}`
})
const seoPrice = computed(() => tour.value?.price)
const seoUrl = computed(() => `${siteUrl}/tours/${slug}`)
...
@media (max-width: 1024px) {
  ...
  .tour-detail-page__booking {
    padding: 1rem !important;
  }
  
  /* Remover sticky do calendário em mobile */
  .tour-detail-page__booking-card {
    position: static !important;
    padding: 0 !important;
  }
  
  /* Calendário com respiro menor no mobile */
  .tour-detail-page__calendar {
    padding: 8px 0;
  }
}
...
@media (max-width: 600px) {
  ...
  .tour-detail-page__content {
    padding: 1rem 0;
  }
  
  .tour-detail-page__hero {
    margin-bottom: 1rem;
    border-radius: 8px;
  }
  ...
  .tour-detail-page__section {
    margin-bottom: 1.5rem;
    word-wrap: break-word;
    overflow-wrap: break-word;
  }
  
  .tour-detail-page__description-html,
  .tour-detail-page__description-text {
    max-width: 100%;
    overflow-x: hidden;
    word-wrap: break-word;
    overflow-wrap: break-word;
  ...
  .tour-detail-page__details-grid {
    grid-template-columns: 1fr;
    gap: 0.75rem;
  }
  
  .tour-detail-page__info-grid {
    grid-template-columns: 1fr;
    gap: 1rem;
  }
  
  .tour-detail-page__includes,
  .tour-detail-page__excludes {
    grid-template-columns: 1fr;
    gap: 0.5rem;
  }
  ...
  .tour-detail-page__participants {
    width: 100%;
    max-width: 100%;
    overflow-x: hidden;
  }
  
  .tour-detail-page__participant-controls {
    width: 100%;
    max-width: 100%;
    overflow-x: hidden;
    display: grid;
  ...
  .tour-detail-page__participant-group {
    width: 100%;
    max-width: 100%;
    overflow-x: hidden;
    flex-direction: column;
  ...
  .tour-detail-page__counter {
    width: 100%;
    max-width: 100%;
    justify-content: center;
  }
  
  .tour-detail-page__counter button {
    min-width: 40px;
    min-height: 40px;
    font-size: 1rem;
  }
  
  .tour-detail-page__counter span {
    min-width: 50px;
    font-size: 1rem;
  }
  
  .tour-detail-page__participant-progress {
    width: 100%;
    max-width: 100%;
    overflow-x: hidden;
  }
  
  .tour-detail-page__progress-bar {
    width: 100%;
    max-width: 100%;
  }
  
  .tour-detail-page__large-group {
    width: 100%;
    max-width: 100%;
    overflow-x: hidden;
  }
  
  .tour-detail-page__whatsapp-buttons {
    grid-template-columns: 1fr;
    gap: 0.5rem;
    width: 100%;
    max-width: 100%;
  ...
  .tour-detail-page__whatsapp-button {
    width: 100%;
    max-width: 100%;
    justify-content: center;
    padding: 0.75rem;
  ...
  .tour-detail-page__price-summary {
    width: 100%;
    max-width: 100%;
    overflow-x: hidden;
  }
  
  .tour-detail-page__price-comparison {
    width: 100%;
    max-width: 100%;
    overflow-x: hidden;
  }
  ...
  .tour-detail-page__subtitle {
    font-size: 1rem;
    line-height: 1.4;
  }
  
  .tour-detail-page__meta {
    flex-direction: column;
    gap: 0.75rem;
    align-items: flex-start;
  }
  
  .tour-detail-page__meta-item {
    font-size: 0.875rem;
  }
  
  .tour-detail-page__price {
    flex-direction: column;
    align-items: flex-start;
    gap: 0.75rem;
  }
  
  .tour-detail-page__price-current {
    font-size: 1.75rem;
  }
  
  .tour-detail-page__cta {
    width: 100%;
    margin-left: 0;
  ...
  .tour-detail-page__schedule {
    grid-template-columns: 1fr;
    gap: 1rem;
  }
  
  .tour-detail-page__schedule-item {
    padding: 1rem;
  }
  
  .tour-detail-page__schedule-icon {
    width: 20px;
    height: 20px;
  }
  
  .tour-detail-page__schedule-content h4 {
    font-size: 1rem;
  }
  
  .tour-detail-page__schedule-content p {
    font-size: 0.875rem;
  }
  
  .tour-detail-page__important-info {
    padding: 1rem;
  }
  
  .tour-detail-page__important-info h3 {
    font-size: 1.25rem;
  }
  
  .tour-detail-page__important-info ul {
    gap: 0.5rem;
  }
  
  .tour-detail-page__important-info li {
    font-size: 0.875rem;
    padding: 0.75rem;
  }
  ...
  .tour-detail-page__booking-card h3 {
    font-size: 1.25rem;
  }
  ...
  .tour-detail-page__participant-label {
    font-size: 0.875rem;
  }
  ...
  .tour-detail-page__counter input {
    min-width: 50px;
    font-size: 1rem;
  }
  ...
  .tour-detail-page__participant-progress h4 {
    font-size: 1rem;
  }
  ...
  .tour-detail-page__participant-feedback {
    padding: 0.75rem;
  }
  
  .tour-detail-page__participant-feedback p {
    font-size: 0.875rem;
  }
...
/* Loading State */
.tour-detail-page__loading {
  padding: 2rem 0;
}

.tour-detail-page__skeleton {
  background: white;
  border-radius: 12px;
  overflow: hidden;
...
.tour-detail-page__skeleton-hero {
  height: 400px;
...
.tour-detail-page__skeleton-content {
  padding: 2rem;
}

.tour-detail-page__skeleton-title {
  height: 2rem;
  background: linear-gradient(90deg, #f0f0f0 25%, #e0e0e0 50%, #f0f0f0 75%);
  background-size: 200% 100%;
  animation: skeleton-loading 1.5s infinite;
...
.tour-detail-page__skeleton-subtitle {
  height: 1.25rem;
  background: linear-gradient(90deg, #f0f0f0 25%, #e0e0e0 50%, #f0f0f0 75%);
  background-size: 200% 100%;
  animation: skeleton-loading 1.5s infinite;
...
.tour-detail-page__skeleton-meta {
  height: 1rem;
  background: linear-gradient(90deg, #f0f0f0 25%, #e0e0e0 50%, #f0f0f0 75%);
  background-size: 200% 100%;
  animation: skeleton-loading 1.5s infinite;
...
/* Error State */
.tour-detail-page__error {
  text-align: center;
  padding: 4rem 2rem;
  color: #666;
}

.tour-detail-page__error svg {
  width: 64px;
  height: 64px;
  color: #dc3545;
  margin-bottom: 1rem;
...
.tour-detail-page__error h2 {
  font-size: 1.5rem;
  margin-bottom: 0.5rem;
  color: #333;
}

.tour-detail-page__error p {
  margin-bottom: 2rem;
}

.tour-detail-page__back {
...
.tour-detail-page__hero-image {
  position: relative;
  height: 400px;
  margin: -2rem -2rem 0 -2rem; /* Compensar padding do info para a imagem */
}

.tour-detail-page__hero-image img {
  width: 100%;
  height: 100%;
  object-fit: cover;
}

.tour-detail-page__badge {
  position: absolute;
  top: 1rem;
  right: 1rem;
  background: var(--brand, #FF6700);
...
.tour-detail-page__discount-badge {
  position: absolute;
  top: 1rem;
  left: 1rem;
  background: #dc2626;
...
/* Galeria de Imagens */
.tour-detail-page__image-gallery {
  display: flex;
  flex-direction: column;
  gap: 1rem;
  height: 100%;
...
.tour-detail-page__image-counter {
  position: absolute;
  bottom: 1rem;
  right: 1rem;
...
.tour-detail-page__thumbnail img {
  width: 100%;
  height: 100%;
  object-fit: cover;
}

.tour-detail-page__thumbnail.active {
  border-color: var(--brand, #FF6700);
  transform: scale(1.05);
}

.tour-detail-page__thumbnail:hover {
  border-color: var(--brand, #FF6700);
  opacity: 0.8;
}

.tour-detail-page__thumbnail-more {
...
.tour-detail-page__single-image img {
  width: 100%;
  height: 100%;
  object-fit: cover;
  border-radius: 12px;
...
.tour-detail-page__lightbox-content {
  position: relative;
  width: 100%;
  max-width: 90vw;
  max-height: 90vh;
...
.tour-detail-page__lightbox-close {
  position: absolute;
  top: -3rem;
  right: 0;
...
.tour-detail-page__lightbox-close:hover {
  background: rgba(255, 255, 255, 0.3);
}

.tour-detail-page__lightbox-nav {
  position: absolute;
  top: 50%;
...
.tour-detail-page__lightbox-prev {
  left: -2rem;
}

.tour-detail-page__lightbox-next {
  right: -2rem;
}

.tour-detail-page__lightbox-nav:hover {
  background: rgba(255, 255, 255, 0.3);
}

.tour-detail-page__lightbox-image {
  max-width: 100%;
  max-height: 70vh;
  overflow: hidden;
  border-radius: 12px;
...
.tour-detail-page__lightbox-image img {
  width: 100%;
  height: 100%;
  object-fit: contain;
}

.tour-detail-page__lightbox-counter {
  color: white;
  font-size: 1rem;
  font-weight: 600;
  text-align: center;
...
.tour-detail-page__lightbox-thumbnails {
  display: flex;
  gap: 0.5rem;
  overflow-x: auto;
  max-width: 100%;
...
.tour-detail-page__lightbox-thumbnail {
  flex-shrink: 0;
  width: 60px;
  height: 45px;
  border: 2px solid transparent;
...
.tour-detail-page__lightbox-thumbnail img {
  width: 100%;
  height: 100%;
  object-fit: cover;
}

.tour-detail-page__lightbox-thumbnail.active {
  border-color: var(--brand, #FF6700);
}

.tour-detail-page__lightbox-thumbnail:hover {
  border-color: var(--brand, #FF6700);
  opacity: 0.8;
}
...
.tour-detail-page__hero-content {
  padding: 2rem;
}
...
.tour-detail-page__schedule-item .iconify {
  width: 16px;
  height: 16px;
  color: var(--brand, #FF6700);
}

.tour-detail-page__location,
.tour-detail-page__rating {
  display: flex;
  align-items: center;
  gap: 0.5rem;
  color: #666;
...
.tour-detail-page__location svg,
.tour-detail-page__rating svg {
  width: 16px;
  height: 16px;
  color: var(--brand, #FF6700);
}
...
.tour-detail-page__price-promo {
  display: flex;
  flex-direction: column;
  gap: 0.25rem;
}

.tour-detail-page__price-original {
  font-size: 1.5rem;
  font-weight: 500;
  color: #9ca3af;
...
.tour-detail-page__price-normal {
  font-size: 2rem;
  font-weight: 700;
  color: var(--brand, #FF6700);
}
...
.tour-detail-page__info-item .iconify {
  width: 24px;
  height: 24px;
  color: var(--brand, #FF6700);
  flex-shrink: 0;
...
.tour-detail-page__info-item h3 {
  font-size: 1rem;
  font-weight: 600;
  color: #333;
  margin-bottom: 0.5rem;
...
.tour-detail-page__detail-item svg {
  width: 24px;
  height: 24px;
  color: var(--brand, #FF6700);
}

.tour-detail-page__detail-item h3 {
  font-size: 0.875rem;
  font-weight: 600;
  color: #333;
...
.tour-detail-page__detail-item p {
  font-size: 0.875rem;
  color: #666;
  margin: 0;
}
...
.tour-detail-page__includes li,
.tour-detail-page__excludes li {
  display: flex;
  align-items: flex-start;
  gap: 0.75rem;
  padding: 0.75rem;
...
.tour-detail-page__includes svg {
  width: 16px;
  height: 16px;
  color: #28a745;
  margin-top: 2px;
...
.tour-detail-page__excludes svg {
  width: 16px;
  height: 16px;
  color: #dc3545;
  margin-top: 2px;
...
.tour-detail-page__itinerary {
  list-style: none;
  padding: 0;
}

.tour-detail-page__itinerary li {
  display: flex;
  align-items: flex-start;
  gap: 1rem;
  padding: 1rem 0;
...
.tour-detail-page__itinerary li:last-child {
  border-bottom: none;
}

.tour-detail-page__step-number {
  display: flex;
  align-items: center;
  justify-content: center;
  width: 32px;
...
.tour-detail-page__selected-dates {
  margin-bottom: 2rem;
  padding: 1rem;
...
.tour-detail-page__selected-dates h4 {
  font-size: 1rem;
  font-weight: 600;
  color: var(--brand, #FF6700);
...
.tour-detail-page__date-info {
  display: flex;
  flex-direction: column;
  gap: 0.5rem;
}

.tour-detail-page__date-item {
  display: flex;
  align-items: center;
  gap: 0.5rem;
  font-size: 0.875rem;
...
.tour-detail-page__date-item svg {
  width: 16px;
  height: 16px;
  color: var(--brand, #FF6700);
}

.tour-detail-page__nights {
  font-size: 0.75rem;
  color: #666;
  font-style: italic;
}
...
.tour-detail-page__participants h4 {
  font-size: 1rem;
  font-weight: 600;
  color: #333;
  margin-bottom: 1rem;
...
.tour-detail-page__participant-group label {
  font-size: 0.875rem;
  font-weight: 500;
  color: #333;
}
...
.tour-detail-page__participant-info {
  display: flex;
  align-items: center;
  gap: 0.5rem;
  padding: 0.5rem;
...
.tour-detail-page__participant-info svg {
  width: 16px;
  height: 16px;
}
...
.tour-detail-page__participant-feedback--info {
  color: #1976d2;
  background: #e3f2fd;
  border: 1px solid #bbdefb;
}

.tour-detail-page__participant-feedback--warning {
  color: #f57c00;
  background: #fff3e0;
  border: 1px solid #ffcc02;
}

.tour-detail-page__participant-feedback--error {
  color: #d32f2f;
  background: #ffebee;
  border: 1px solid #ffcdd2;
}

.tour-detail-page__participant-feedback svg {
  width: 16px;
  height: 16px;
}
...
.tour-detail-page__progress-fill {
  height: 100%;
...
.tour-detail-page__progress-fill::after {
  content: '';
  position: absolute;
  top: 0;
  left: 0;
...
.tour-detail-page__progress-text {
  font-size: 0.875rem;
  color: #666;
  text-align: center;
  font-weight: 500;
...
.tour-detail-page__counter button:hover:not(:disabled) {
  border-color: var(--brand, #FF6700);
  color: var(--brand, #FF6700);
}

.tour-detail-page__counter button:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}
...
.tour-detail-page__auto-save-status {
  display: flex;
  align-items: center;
  justify-content: space-between;
  margin-bottom: 1rem;
...
.tour-detail-page__save-indicator {
  display: flex;
  align-items: center;
  gap: 0.5rem;
  font-size: 0.875rem;
...
.tour-detail-page__save-indicator--blue {
  color: #1976d2;
}

.tour-detail-page__save-indicator--orange {
  color: #f57c00;
}

.tour-detail-page__save-indicator--green {
  color: #2e7d32;
}

.tour-detail-page__save-indicator--gray {
  color: #666;
}

.tour-detail-page__save-indicator svg {
  width: 16px;
  height: 16px;
}

.tour-detail-page__clear-save {
...
.tour-detail-page__clear-save:hover {
  background: rgba(220, 53, 69, 0.1);
}

.tour-detail-page__clear-save svg {
  width: 14px;
  height: 14px;
}
...
.tour-detail-page__description-html :deep(img) {
  max-width: 100%;
  height: auto;
  border-radius: 8px;
  margin: 1rem 0;
...
.tour-detail-page__description-html :deep(table) {
  width: 100%;
  border-collapse: collapse;
  margin: 1rem 0;
}

.tour-detail-page__description-html :deep(th),
.tour-detail-page__description-html :deep(td) {
  padding: 8px 12px;
...
.tour-detail-page__description-html :deep(a[href]) {
  color: var(--brand);
  text-decoration: underline;
}

.tour-detail-page__description-html :deep(a[href]:hover) {
  color: var(--brand-600);
}

.tour-detail-page__description-html :deep(h1),
.tour-detail-page__description-html :deep(h2),
.tour-detail-page__description-html :deep(h3),
.tour-detail-page__description-html :deep(h4),
.tour-detail-page__description-html :deep(h5),
.tour-detail-page__description-html :deep(h6) {
  margin: 1.5rem 0 0.5rem 0;
  font-weight: 600;
  color: var(--text);
}

.tour-detail-page__description-html :deep(h1) { font-size: 1.875rem; }
.tour-detail-page__description-html :deep(h2) { font-size: 1.5rem; }
.tour-detail-page__description-html :deep(h3) { font-size: 1.25rem; }
.tour-detail-page__description-html :deep(h4) { font-size: 1.125rem; }
.tour-detail-page__description-html :deep(h5) { font-size: 1rem; }
.tour-detail-page__description-html :deep(h6) { font-size: 0.875rem; }

.tour-detail-page__description-html :deep(ul),
.tour-detail-page__description-html :deep(ol) {
  margin: 1rem 0;
  padding-left: 1.5rem;
}

.tour-detail-page__description-html :deep(li) {
  margin: 0.25rem 0;
}

.tour-detail-page__description-html :deep(blockquote) {
...
.tour-detail-page__description-html :deep(pre) {
  background-color: #f9fafb;
  padding: 1rem;
  border-radius: 8px;
  overflow-x: auto;
...
.tour-detail-page__description-html :deep(code) {
...
/* não cortar a imagem */
.tour-detail-page__main-image-img {
  width: 100% !important;
...
.tour-detail-page__nav:hover {
  background: rgba(0,0,0,.65);
}

.tour-detail-page__nav.prev {
  left: 10px;
}

.tour-detail-page__nav.next {
  right: 10px;
}
...
.tour-detail-page__price-regular {
  font-size: clamp(14px, 1.6vw, 18px);
  color: #9aa1a8;
  text-decoration: line-through;
}

.tour-detail-page__price-badge {
  background: #ffe8dc;
  color: #ff6a00;
  font-weight: 700;
  padding: 4px 8px;
...
.tour-detail-page__large-group-content {
  display: flex;
  align-items: center;
  gap: 0.75rem;
}

.tour-detail-page__large-group-icon {
  width: 24px;
  height: 24px;
  color: #d97706;
  flex-shrink: 0;
...
.tour-detail-page__large-group-text h5 {
  font-size: 1rem;
  font-weight: 600;
  color: #92400e;
...
.tour-detail-page__large-group-text p {
  font-size: 0.875rem;
  color: #a16207;
  margin: 0;
  line-height: 1.4;
...
.tour-detail-page__whatsapp-button:hover {
  background: #20ba5a;
  transform: translateY(-1px);
  box-shadow: 0 4px 8px rgba(37, 211, 102, 0.3);
}

.tour-detail-page__whatsapp-button .icon {
  width: 16px;
  height: 16px;
}
...
.tour-detail-page__nav.prev { left: 38px; }
.tour-detail-page__nav.next { right: 38px; }
...
@media (hover:hover) and (pointer:fine) {
  ...
  .tour-detail-page__main-image:hover .tour-detail-page__nav { 
    opacity: 1; 
  }
}
...
@media (max-width: 768px) {
  ...
  .tour-detail-page__nav.prev { left: 32px; }
  .tour-detail-page__nav.next { right: 32px; }
